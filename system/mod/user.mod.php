...
class mod_user extends mod {
	...
	public function setting_login_password() {
		...
		if (!$this->db->query($sql, 0)) core::error('更新[新登录密码]失败，请重试');
	...
	public function setting_coin_password() {
		...
		if (!$this->db->query($sql, 0)) core::error('更新[新资金密码]失败，请重试');
	...
	private function pay_online($bankid, $amount, $orderid) {
		...
		if ($bankid === '1') { // 支付宝
			$html  = '<form name="alipaypay" method="post" action="/alipay.png">';
