$(function() {
	...
	window.lottery = {
		...
		display_code_list: function(code) {
			$.dialogue({
				class: 'mid',
				body: '<textarea style="height:80px;color:#666;padding:10px;width:548px;border:1px solid #ddd">' + code.actionData + '</textarea>',
				yes: {text: '我知道了'},
			});
		},
		// 追号
		...
		game_post_code: function() {
			...
			loadpage.before(function() {
				...
				html += '<div class="submit-bets-title icon-basket">确定要购买第<span class="btn btn-green">' + current.actionNo + '</span>期彩票？</div>';
				html += '<div class="submit-bets-info">';
				html += '<div class="count left icon-chart-bar">总投注数：<span class="num">' + $('#all-count').text() + '</span>注</div>';
				html += '<div class="amount left icon-yen">购买金额：<span class="num">' + $('#all-amount').text() + '</span>元</div>';
				html += '</div>';
				html += '</div>';
				html += '<div class="submit-bets-body">';
				html += $('#bets-cart').html().replace('<td>操作</td>', '').replace(/<td><a [^>]*?>删除<\/a><\/td>/g, '');
				html += '</div>';
				html += '</div>';
				loadpage.close(function() {
					$.dialogue({
		...
		countdown: function(diffTime, actionNo) {
			...
			if (diffTime <= 0) {
				...
				$.dialogue({
					type: 'success',
					text: '当前期结束，点击<span class="btn btn-blue">确定</span>清空已投注内容并刷新页面，点击<span class="btn btn-white">取消</span>不刷新页面',
					auto: true,
					yes: {
						text: '确定',
						func: function() {
		...
		fresh: function() {
			...
			if (lottery.timer.moveno) clearInterval(lottery.timer.moveno);
			$.load('/game/lottery?id=' + game.type, '#game-lottery .lottery-container');
		},
		/**
		 * @name 获取本期盈亏
		 * @param int type 彩种ID
