...
function addUserCountModal(){
	$.get('/index.php/member/addUserCount', function(html){
...
function memberAddUserCount(err, data){
	if(err){
		error(err);
	}else{
		if(data) success(data);
		$(this).closest('.UserCountAdd').dialog('destroy').remove();
...
function _onUpdateCompile2(type, msg){
	var err, data;
	$('.bank-modal').dialog('destroy').remove();
	if(type=='error'){
		err=msg;
