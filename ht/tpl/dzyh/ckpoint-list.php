...
<input type="hidden" value="<?=$this->user['username']?>" />
    <header>
    	<h3 class="tabs_involved">存款记录
            <div class="submit_link wz">
            	<form action="/index.php/dzyh/ckpointList" target="ajax" call="defaultSearch" dataType="html">
	...
	<tbody> 
		...
		<tr> 
			...
			<td><a href="/index.php/dzyh/dzyhpointedit/<?=$var['id']?>" method="post" width="400" class="myanswer" target="modal" title="存款编辑" modal="true" call="pointHandle" dataType="json">编辑</a>|
			    <?if(!$this->getValue("select enable from {$this->prename}Dzyh_ck_swap where id={$var['id']}")){?>
			    <a href="/index.php/dzyh/dzyhpointdj/<?=$var['id']?>" method="post" target="ajax" call="pointHandle" title="冻结存款后用户无法取出，是否继续？" dataType="json">冻结</a></td>
				<?}else{?>
				<a href="/index.php/dzyh/dzyhpointjd/<?=$var['id']?>" method="post" target="ajax" call="pointHandle" title="即将解冻该笔春款，是否继续？" dataType="json">解冻</a></td>
				<?}?>
		</tr>
		<?php }else{ ?>
			<tr>
