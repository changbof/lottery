<?php
	$sql="select u.*, sum(b.mode * b.beiShu * b.actionNum) betAmount, sum(b.bonus) zjAmount, (select sum(c.amount) from lottery_member_cash c where c.`uid`=u.`uid` and c.state=0 ) cashAmount,(select sum(r.amount) from lottery_member_recharge r where r.`uid`=u.`uid` and r.state in(1,2,9) ) rechargeAmount from lottery_members u left join lottery_bets b on u.uid=b.uid and b.isDelete=0 where 1";
	
	// 用户名限制
	// 从属关系限制
	if($_GET['username']&&$_GET['username']!="用户名"){
		$_GET['username']=wjStrFilter($_GET['username']);
		if(!preg_match('/^\w{4,16}$/',$_GET['username'])) throw new Exception('用户名包含非法字符,请重新输入');
	...
	$sql="select sum(coin) from lottery_coin_log where uid=? and liqType between 2 and 3";
	$sql2="select * from lottery_member_session where uid=? order by id desc limit 1";
?>
...
?>
    ...
    <tr> 
        ...
        <td><a href="/index.php/Member/userAmount/<?=$var['uid']?>" target="modal"  width="420" title="用户统计" modal="true">统计</a> | <a href="business/coinLog?username=<?=$var['username']?>">帐变</a> | <a href="/index.php/Member/userUpdate/<?=$var['uid']?>" target="modal"  width="420" title="编辑用户" modal="true" button="确定:dataAddCode|取消:defaultCloseModal">编辑</a> | <a href="member/index?type=2&uid=<?=$var['uid']?>">下级</a> | <a href="/index.php/Member/delete/<?=$var['uid']?>" target="modal"  width="320" title="删除用户" modal="true" button="确定:dataAddCode">删</a> | <a href="/index.php/Member/removeEdge/<?=$var['uid']?>" target="modal"  width="320" title="将下线转移到其他用户名下" modal="true" button="确定:dataAddCode">转</a>  |  <a method="post" target="ajax" call="TSuccess" title="是否踢该会员下线?" href="/index.php/System/T/<?=$var['uid']?>">踢下线</a></td> 
    </tr>
   <?php }else{?>
    <tr> 
	...
	<?php }else{ ?>
		<tr>
			<td colspan="12">没用用户或该用户还没有下级</td>
		</tr>
	<?php } ?>
</tbody> 
</table>
