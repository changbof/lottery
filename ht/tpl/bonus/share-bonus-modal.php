<?php
	...
	$sql="select uid,username,fanDian from lottery_members WHERE uid=?";
	...
	$fanDian = floatval($bonusUser['fanDian']);
	if ($fanDian >= $bonusScale_fandian1_min && $fanDian < $bonusScale_fandian1_max) {
		...
		die('<strong style="color:red;font-size:14px;">返点比例不符合系统设置</strong>');
	}

	if($bonusUser['fanDian'] == 8.0){ 
		$bonusUser['bonusScale'] = $this->settings['bonusScale1']; 
	}else{ 
		$bonusUser['bonusScale'] = $this->settings['bonusScale2'];
	}
