<?php
	...
	$bonusScale_fandian1_min = floatval($this->settings['bonusScale_fandian1_min']);
	$bonusScale_fandian1_max = floatval($this->settings['bonusScale_fandian1_max']);
	$bonusScale_fandian2_min = floatval($this->settings['bonusScale_fandian2_min']);
	$bonusScale_fandian2_max = floatval($this->settings['bonusScale_fandian2_max']);
	
	$sqlFandian = $bonusScale_fandian1_min > $bonusScale_fandian2_min ? $bonusScale_fandian2_min : $bonusScale_fandian1_min;
	
	$sql="select uid,username,fanDian from lottery_members WHERE fanDian >= $sqlFandian";
...
<article class="module width_full">
	...
	<tbody>
		<?php
			if($data) {
				foreach($data as $var){
					$bonusId = $this->getValue('select id from lottery_bonus_log where uid='.$var['uid'].' and startTime='.$fromTime.' and endTime = '.$toTime);
					if($bonusId) continue;
					$var['lossAmount'] = $this->statLossAmount($var['uid'],$fromTime,$toTime);
					if (!$var['lossAmount']) continue;
					$fanDian = floatval($var['fanDian']);
					if ($fanDian >= $bonusScale_fandian1_min && $fanDian < $bonusScale_fandian1_max) {
						$var['bonusScale'] = $this->settings['bonusScale1'];
					} else if ($fanDian >= $bonusScale_fandian2_min && $fanDian < $bonusScale_fandian2_max) {
