<?php
	...
	$sql="select u.username, u.coin, u.uid, u.parentId, sum(b.mode * b.beiShu * b.actionNum) betAmount, sum(b.bonus) zjAmount, (select sum(c.amount) from lottery_member_cash c where c.state=0 and c.`uid`=u.`uid` $cashTimeWhere) cashAmount,(select sum(r.amount) from lottery_member_recharge r where r.state in(1,2,9) and r.`uid`=u.`uid` $rechargeTimeWhere) rechargeAmount, (select sum(l.coin) from lottery_coin_log l where l.liqType in(50,51,52,53,54,56,130,120,110) and l.`uid`=u.`uid` $brokerageTimeWhere) brokerageAmount from lottery_members u left join lottery_bets b on b.isDelete=0 and u.uid=b.uid $betTimeWhere where 1 $userWhere";

	$this->pageSize-=1;
	if($this->action!='betDate') $this->action='betDate';
	//$list=$this->getPage($sql .' group by u.uid', $this->page, $this->pageSize);
	$list=$this->getPage($sql .' group by u.uid', $this->page, 20);
	if(!$list['total']) {
	...
	$sql2="select sum(b.mode * b.beiShu * b.actionNum) betAmount, sum(b.bonus) zjAmount from lottery_members u left join lottery_bets b on b.isDelete=0 and u.uid=b.uid $betTimeWhere $userWhere3";
	...
	$all['brokerageAmount']=$this->getValue("select sum(l.coin) from {$this->prename}coin_log l, {$this->prename}members u where l.liqType in(50,51,52,53,54,56,130,120,110) and l.uid=u.uid $brokerageTimeWhere $userWhere3", $var['uid']);
...
<input type="hidden" value="<?=$this->user['username']?>" />
	...
	<?php
		...
		if($var['parentId'] != NULL){continue;}
		
		if($var['username']!='没有下级了'){
			$var['fanDianAmount']=$this->getValue($sql, $var['uid']);
			$pId=$var['uid'];
			$var['teamwin']=$this->getValue("select sum(l.coin) fandianAll from {$this->prename}coin_log l, lottery_members u where l.liqType in(2,3) and l.uid=u.uid and concat(',', u.parents, ',') like '%,$pId,%' $fanDiaTimeWhere") + $this->getValue("select sum(b.bonus-b.mode * b.beiShu * b.actionNum) betZjAmount from lottery_members u ,lottery_bets b where b.isDelete=0 and u.uid=b.uid and concat(',', u.parents, ',') like '%,$pId,%' $betTimeWhere");
		}
...
<footer>
	<?php
		$rel=get_class($this).'/betDates-{page}?'.http_build_query($_GET,'','&');
