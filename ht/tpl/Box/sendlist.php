<?php
	...
	$sql="select s.mid, s.title, s.content, s.from_username, s.time from {$this->prename}message_sender s, {$this->prename}admin_members a where s.from_uid={$this->user['uid']} and s.from_deleted=0 $timeWhere and s.from_uid=a.uid order by s.time DESC";
...
<?php }else{ ?>
    ...
    <tr>
    	<td colspan="6" style="text-align:left; padding-left:10px;">
           ...
           <a href="/index.php/box/senddelete/" dataType="json" class="removeAllRecord" onajax="sendrecordBeforeDelete" call="senddeleteBet" target="ajax"><input name="delall" type="button" value="删 除" /></a>
		</td>
    </tr>
</tbody> 
</table>
