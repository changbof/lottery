<?php
	...
	$sql="select s.mid, r.is_readed, s.title, s.content, s.from_username, s.time from {$this->prename}message_sender s, {$this->prename}message_receiver r where r.to_uid=0 and r.is_deleted=0 $timeWhere $stateWhere $userWhere and r.mid=s.mid order by s.time DESC";
...
<?php
	if($data['data']) foreach($data['data'] as $var){?>
        <tr>
			...
			<td><a href="/index.php/box/midinfo/<?=$var['mid']?>" button="" title="消息内容" width="510" modal="true" target="modal"><?=$var['mid']?></a></td>
			...
			<td><div class="answer"><a href="/index.php/box/answer/<?=$var['mid']?>" class="myanswer" target="modal" width="600" title="回复" modal="true" button=""><input value="回 复" class="bnt" type="button"></a></div></td>
		</tr>
   <?php }else{ ?>
		<tr>
	...
	<tr>
    	<td colspan="8" style="text-align:left; padding-left:10px;">
           ...
           <a href="/index.php/box/receivelist" dataType="json" class="removeAllRecord" onajax="receiverecordBeforeDelete" call="receivedeleteBet" target="ajax"><input name="delall" type="button" value="删 除" /></a>
		</td>
    </tr>
</tbody> 
</table>
