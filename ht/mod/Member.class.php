...
public final function fabuxiaoxi_action(){
	...
	public final function added($passwordEncrypt=false){
		...
		if(!preg_match('/^\w{4,16}$/', $update['username'])) throw new Exception('用户名只能是由字母、数字、下划线、横杠组成的4~16位字符');
	...
	public final function removeEdge($uid) {
		$uid = intval($uid);
		$this->display('member/removeEdge-modal.php', 0, $uid);
	}
	
	private function getParentData($parents) {
		$result = array();
		$data = explode(',', $parents);
		foreach ($data as $v) {
			$v = trim($v);
			if (!empty($v)) array_push($result, $v);
		}
		return $result;
	}
	
	public final function removeEdged($uid) {
		$uid = intval($uid);
		$target = trim($_POST['target']);
		if (!is_numeric($target)) throw new Exception('目标用户ID必须为数字');
		$new_user = $this->getRow("SELECT `uid`,`parents` FROM `{$this->prename}members` WHERE `uid`=$target LIMIT 1");
		if (!$new_user) throw new Exception('您输入的用户ID错误(不存在该用户)');
		$new_parents = trim($new_user['parents']);
		$new_parents_data = $this->getParentData($new_parents);
		$old_user = $this->getRow("SELECT `uid`,`parents` FROM `{$this->prename}members` WHERE `uid`=$uid LIMIT 1");
		$old_parents = trim($old_user['parents']);
		$old_parents_data = $this->getParentData($old_parents);
		// 获取用户的所有下级
		$sons = $this->getRows("SELECT `uid`,`parentId`,`parents` FROM `{$this->prename}members` WHERE CONCAT(',', parents, ',') LIKE '%,$uid,%'");
		foreach ($sons as $son) {
			$this_uid = $son['uid'];
			if ($son['parentId'] == $uid) {
				$this_new_parentId = $target;
				$this_new_parents = $this_uid.','.$new_parents;
			} else {
				$this_new_parentId = $son['parentId'];
				$this_parents_data = $this->getParentData($son['parents']);
				$this_new_parents = '';
				foreach ($this_parents_data as $v) {
					if (!in_array($v, $old_parents_data)) $this_new_parents .= $v.',';
				}
				foreach ($new_parents_data as $v) $this_new_parents .= $v.',';
				$this_new_parents = substr($this_new_parents, 0, -1);
			}
			$sql = "UPDATE `{$this->prename}members` SET `parentId`=$this_new_parentId,`parents`='$this_new_parents' WHERE `uid`=$this_uid LIMIT 1";
			$this->update($sql);
		}
		$sql = "UPDATE `{$this->prename}members` SET `parentId`=$target,`parents`='$uid,$new_parents' WHERE `uid`=$uid LIMIT 1";
		$this->update($sql);
		return '已将该用户转为['.$target.']的下级';
	}
...
//用户等级限额
	...
	// 添加用户等级限额
	public final function addUserCount() {
		$this->display('member/addUserCount-modal.php');
	}
	
	public final function addUserCount_func() {
		$fanDian = array_key_exists('fanDian', $_POST) ? floatval(number_format($_POST['fanDian'], 1)) : 0;
		$userCount = array_key_exists('userCount', $_POST) ? floatval($_POST['userCount']) : 0;
		if (!$fanDian) throw new Exception('请输入返点');
		if (!$userCount) throw new Exception('请输入用户限额');
		if ($this->getRow("SELECT `id` FROM `{$this->prename}params_fandianset` WHERE `fanDian`={$fanDian} LIMIT 1")) {
			throw new Exception('您输入的返点已存在');
		}
		if ($this->insertRow($this->prename .'params_fandianset', array('fanDian' => $fanDian, 'userCount' => $userCount))) {
			return '添加用户限额成功';
		} else {
			throw new Exception('未知出错');
		}
