...
class Bonus extends AdminBase{
	...
	public final function shareBonusSingle($uid){
		...
		$para['uid'] = $uid;
		$para['username'] = $this->getValue('select username from lottery_members where uid=?',$uid);
		
		$sql = 'select id from lottery_bonus_log where uid='.$uid.' and startTime='.$para['startTime'].' and endTime='.$para['endTime'];
		...
		if($this->insertRow('lottery_bonus_log',$para)){
	...
	public function statLossAmount($uid,$fromTime,$toTime){
		...
		$sql="select u.username, u.coin, u.uid, u.parentId, sum(b.mode * b.beiShu * b.actionNum) betAmount, sum(b.bonus) zjAmount, (select sum(l.coin) from lottery_coin_log l where l.`uid`=u.`uid` and l.liqType in(50,51,52,53,56) $brokerageTimeWhere) brokerageAmount from lottery_members u left join lottery_bets b on u.uid=b.uid and b.isDelete=0 $betTimeWhere where 1 $userWhere";

		$homeUdata=$this->getRows($sql .' group by u.uid');
		$homeUid = $homeUdata[0]['uid'];
		$childUidarr = $this->getRows("select uid from {$this->prename}members where parentId=$homeUid");
		...
		foreach($childUidarr as $child){
				$childrenarr = $this->getRows("SELECT uid FROM `lottery_members` WHERE parents LIKE '%$child[uid]%'");
				$childrenUids = implode(',',array_column($childrenarr,'uid'));
				$childrenWhere = 'and u.uid in('.$childrenUids.') ';

				$childarr=$this->getRow("select sum(b.mode * b.beiShu * b.actionNum) betAmount, sum(b.bonus) zjAmount from lottery_members u left join lottery_bets b on u.uid=b.uid and b.isDelete=0 $betTimeWhere $childrenWhere");
