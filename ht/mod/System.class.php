...
class System extends AdminBase{
	...
	public final function updateSettings(){
		...
		if(!is_numeric($para['betMaxAmount'])) throw new Exception('请正确设置每人每期最大投注金额限制!'); // add by aboooo at 20160821
		if(!ctype_digit($para['betMaxAmount'])) throw new Exception('请正确设置每人每期最大投注金额限制!');    
	...
	public final function updateBank(){
		...
		$sql="insert into {$this->prename}admin_bank set";
		foreach($para as $field=>$var){
			$sql.=" `$field`='$var',";
		}
		$sql=rtrim($sql,',');
		$sql.=' on duplicate key update `bankid`=values(`bankid`), `account`=values(`account`), `username`=values(`username`), `enable`=values(`enable`)';
	...
	public final function updateBanklist(){
		...
		$sql=rtrim($sql,',');
		$sql.=' on duplicate key update id=values(id), `name`=values(`name`), `sort`=values(`sort`), `isDelete`=values(`isDelete`)';
		...
		echo '<script type="text/javascript">top._onUpdateCompile2("', $fun, '", ', json_encode($msg), ')</script>';
	}
